...
2. Consider a matrix $\bA \in \R^{n \times m}$. Which of the following are necessary conditions for $\bA$ to be invertible? Which are sufficient? 

    a. $\bA$ is a square matrix, i.e. $n = m$  
    b. The columns of $\bA$ span $\R^{n}$ 
    c. The columns of $\bA$ are linearly independent 
    d. $\bA$ has no linearly dependent columns 
    e. The null-space of $\bA$ is empty 
    f. The only solution of $\bA \bx = 0$ is $\bx = 0$. 
<!-- What properties does $\bA$ need to satisfy in order for $\bA$ to be invertible? *Hint: there are many properties of a matrix that show it is uniquely invertible. Think dimensions, fundamental spaces, $\bA^{-1} \bA = ?$, etc.*  -->
